...
const Purchase = () => {
  ...
  const { data, isLoading, refetch } = useQuery("users", () =>
    fetch(`https://protected-cliffs-14570.herokuapp.com/tools/${id}`, {
      method: "get",
      // headers: {
  ...
  const validateNum = () => {
    if (qtyRef.current.value < data.minorder) {
      qtyRef.current.value = data.minorder;
    } else if (qtyRef.current.value > data.available) {
      qtyRef.current.value = data.available;
    }
  };
  // useEffect(()=>{
  //   const url = `https://protected-cliffs-14570.herokuapp.com/tools/${id}`;
  //   fetch(url)
  //     .then((res) => res.json())
  //     .then((item) => {
  //       setData(item);
  ...
  const quantityHandle = (e) => {
    ...
    fetch("https://protected-cliffs-14570.herokuapp.com/order", {
      method: "post",
      headers: {
      ...
      <div class="hero min-h-screen bg-base-200">
        <div class="hero-content flex-col lg:flex-row">
          <img
            src={data?.img}
            alt="pic"
            class="mask mask-squircle w-3/4 md:w-96 "
          />
          <div className="p-5">
