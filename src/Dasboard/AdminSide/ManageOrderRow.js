...
const ManageOrderRow = ({ order, index, refetch }) => {
  ...
  const changeOderStatus = (id) => {
    alert(id);
    fetch(`https://protected-cliffs-14570.herokuapp.com/shippedorder/${id}`, {
      method: "PATCH",
      headers: {
  ...
  return (
    <tr>
      ...
      <td title={desc}>
        {decLength > 20 ? desc.slice(0, 20) + "......." : desc}
      </td>
      <td>{qnty}</td>
      <td>{total}</td>
      <td>
        ...
        {paid && shipped && (
          <button class="btn btn-xs btn-success text-white">Shipped</button>
        )}
