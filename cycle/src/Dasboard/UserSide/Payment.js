...
const Payment = () => {
    const {id} = useParams();
    const url =`http://localhost:4000/particularOrder/${id}`;
    const{data:payorder,isLoading} = useQuery(['singleorder',id],()=>fetch(url,{
        method: "get",
                headers: {
...
<div class="hero min-h-screen bg-base-200">
  ...
  <h2 class="text-2xl">Hello ! {payorder.user} </h2>
    ...
    <p className='text-3xl text-gray-800'>Total Bill: ${payorder.total}</p>
